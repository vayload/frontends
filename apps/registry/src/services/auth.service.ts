...
class AuthService {
	...
	public async loginWithPassword(email: string, password: string): Promise<Result<AuthResponse, HttpError>> {
		const response = await httpClient.reqwest().post<any>('/auth/login', { email, password });
	...
	public async loginWithOAuth(provider: string, origin: string): Promise<Result<{ url: string }, HttpError>> {
		...
		const response = await httpClient.reqwest().post<any>(`/auth/oauth/${provider}`, {
			origin_uri: origin,
			state: pkce.state,
		...
		return response.map((res) => ({
			url: res.data.authorization_uri,
		}));
	}

	public async register(input: RegisterInput): Promise<Result<AuthResponse, HttpError>> {
		const response = await httpClient.reqwest().post<any>('/auth/register', {
