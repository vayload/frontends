...
import type { PrivateUserProfile, UpdateProfileDto } from '../models';
...
class UsersService {
	...
	public async getUserStats(
		...
		const response = await httpClient.reqwest().get<any>(`/users/${id}/stats`);
		return response.map((res) => ({
			pluginsCount: res.data.data.plugins_count,
